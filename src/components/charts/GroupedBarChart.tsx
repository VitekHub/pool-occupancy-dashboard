...
import { useMultiWeekData } from '@/utils/hooks/useMultiWeekData';
...
const GroupedBarChart: React.FC<GroupedBarChartProps> = ({ selectedWeekId }) => {
  const { t, i18n } = useTranslation(['charts', 'common']);
  const [selectedDay, setSelectedDay] = useState<string>('Monday');
  const [startHourIndex, setStartHourIndex] = useState(0);
  const { availableWeeks, weeklySummaries, weekCapacityData, loading, error } = useMultiWeekData();
    ...
    <div className="bg-white p-6 rounded-lg shadow-lg">
      ...
      <div className="mt-8 flex items-center">
        ...
        <button
          ...
          className={`p-2 rounded-lg ${
            startHourIndex >= validHours.length - 3
              ? 'text-gray-400 cursor-not-allowed'
              : 'text-gray-700 hover:bg-gray-100'
          }`}
